...
def write_assemble_makeflow(makeflow,samples,aligndir,assembledir,gtf,thread):
    ...
    fo.write("\t%s --merge -G %s -p %d -o %s %s "%(stringtie,gtf,thread,os.path.join(assembledir,"merge/merged.gtf")," ".join([os.path.join(assembledir,s,"assemble.gtf") for s in samples])))
...
def write_get_count_matrix_makeflow(makeflow,samples,assembledir):
    prepDE = RNA.get_bin_abspath("prepDE.py")
    fo = open(makeflow,"a+")
    fo.write("CATEGORY=get_count_matrix\n")
    fo.write(os.path.join(assembledir,"gene_count_matrix.txt ") + os.path.join(assembledir,"transcript_count_matrix.txt : ") + " ".join([os.path.join(assembledir,sn,"transcripts.gtf") for sn in samples]) + "\n")
...
def write_get_expression_matrix_makeflow(makeflow,samples,assembledir,type):
    ...
    fo.write("\tpython %s -i %s -o %s -t %s &> %s\n\n"%(get_expression_matrix," ".join([os.path.join(assembledir,sn,"gene_abundance.txt") for sn in samples]),os.path.join(assembledir,"gene.expression.txt"),type,os.path.join(assembledir,"get_gene_expression.log")))
    fo.write("CATEGORY=get_isoform_expression_matrix\n")
    fo.write(os.path.join(assembledir,"isoform.expression.txt : ") + " ".join([os.path.join(assembledir,sn,"transcripts.gtf") for sn in samples]) + "\n")
    fo.write("\tpython %s -i %s -o %s -t %s &> %s\n\n"%(get_expression_matrix," ".join([os.path.join(assembledir,sn,"transcripts.gtf") for sn in samples]),os.path.join(assembledir,"isoform.expression.txt"),type,os.path.join(assembledir,"get_isoform_expression.log")))
    fo.close()
...
def write_summary_trans_stat_makeflow(makeflow,samples,assembledir):
    ...
    fo.write(" ".join([os.path.join(assembledir,sn,name) for sn in samples+["merge"] for name in ["transcripts.gtf.length_stat","transcripts.gtf.transcript.length_distribution"]]) + " : " + " ".join([os.path.join(assembledir,sn,'transcripts.gtf') for sn in samples+["merge"]]) + "\n")
    fo.write("\tpython %s -i %s -o %s &> %s\n\n"%(summary_trans," ".join([os.path.join(assembledir,sn,'transcripts.gtf') for sn in samples+["merge"]]),os.path.join(assembledir,"assembly.state.txt"),os.path.join(assembledir,"summary_trans.log")))
    fo.close()
    
def write_filter_feature_makeflow(makeflow,assembledir,gtf):
    filter_new_feature = RNA.get_bin_abspath("filter_new_feature.py")
    fo = open(makeflow,"a+")
    fo.write("CATEGORY=filter_new_feature\n")
    fo.write(" ".join([os.path.join(assembledir,i) for i in ["gene_count_matrix.txt.all","gene.expression.txt.all","isoform.expression.txt.all","transcript_count_matrix.txt.all"]]) + " : " + " ".join([os.path.join(assembledir,i) for i in ["gene_count_matrix.txt","gene.expression.txt","isoform.expression.txt","transcript_count_matrix.txt"]]) + "\n")
...
def main():
    ...
    write_assemble_makeflow(args.makeflow,samples,aligndir,assembledir,gtf,args.parallel)
    ...
    write_get_expression_matrix_makeflow(args.makeflow,samples,assembledir,args.type)
    if not args.noassemble:
        write_filter_feature_makeflow(args.makeflow,assembledir,gtf)
