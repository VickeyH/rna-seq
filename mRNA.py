...
def parseArg():
    ...
    parser.add_argument("-a","--addition",type=str,help="the supplyment sample name file, first colnum is the sample name that will mathch the rawdata filename, second colnum is the final sample name which will be uesd for downstream analysis, default:None")
    ...
    parser.add_argument("-anno","--anno",type=str,help="the genome reference anno file",required=True)
    ...
    parser.add_argument("-dt","--database_type",type=str,default="/K/G",help="enrichment database_type, default: /K/G, this can be get from '/lustre/work/ranlv/program/kobas3.0.3/kobas/kobas3.0_type.txt' file")
    parser.add_argument("-it","--idtype",type=str,default="id:ncbigene",choices =["id:ncbigi", "id:uniprot", "id:ensembl","id:ncbigene"],help='gene type of back list and deg list, can be chosen from ["id:ncbigi", "id:uniprot", "id:ensembl","id:ncbigene"],default: id:ncbigene, infomation can be found in "/lustre/work/ranlv/program/kobas3.0.3/kobas/kobas3.0_type.txt" file.',metavar = "idtype")
    parser.add_argument("-abbr","--abbr",type=str,help="species abbr, can be found in '/lustre/work/ranlv/program/kobas3.0.3/kobas/kobas3.0_type.txt' file, if --nokobas, do not need this option.")
...
def chech_arg():
    args = parseArg()
    if not os.path.exists(args.fa) or not os.path.exists(args.gtf) or not os.path.exists(args.anno):
        print "Error: fa or gtf or anno file not exists"
        sys.exit(1)
    ...
    #    if not args.nokobas:
    #        if not args.abbr:
    #            print "Error: --abbr must set for enrichment analysis"
    #            flag = False
    #        elif len(args.abbr) != 3 or len(args.abbr) != 4:
    #            print "Error: --abbr must be well setting"
    #            flag = False
    #        else:
    #            print "Error: --abbr must be well setting"
    #            flag = False
...
def check_makeflow(makeflow):
    ...
    if out != "All need files exists, OK!":
        print out + "\tin check makeflowfile [%s]"%makeflow
        sys.exit(1)
...
def main():
    ...
    if os.path.exists(join(out,args.makeflow)):
        print "Error: %s file exists, please use a new name"%args.makeflow
        sys.exit(1)
    ...
    mkdir(analysis,[moduledict.get(i) for i in modules])
    ...
    fa = abspath(args.fa);gtf=abspath(args.gtf);anno=abspath(args.anno)
    ...
    if "align" in modules:
        cmdalign = "python /lustre/work/yongdeng/software/eukaryon/flow/hisat_align_clean.py -i %s -fa %s -gtf %s -p %d -o %s -mf %s"%(join(analysis,"cleandata"),fa,gtf,args.parallel,join(analysis,"align"),join(out,"flow",now+"flow","Align.mf"))
        if args.noassemble:
            cmdalign += " -a"
    ...
    if "cluster" in modules:
        vsdeg = [i+","+join(analysis,"deg",i,"deg.up_down.txt") for i in [v.split(",")[0] for v in args.vs]]
    ...
    if "coexpression" in modules:
        cmdcoe = "python /lustre/work/yongdeng/software/protokaryon/flow/coexpression.py -eq %s -deg %s -vs %s -sg %s -o %s -mf %s"%(join(analysis,"stringtie/gene_FPKM_matrix.txt"),join(analysis,"deg")," ".join([v.split(",")[0] for v in args.vs])," ".join(args.sample_group),join(analysis,"coexpression"),join(out,"flow",now+"flow","Coexpression.mf"))
        os.system(cmdcoe);shell.append(cmdcoe);mf.append(join(out,"flow",now+"flow","Coexpression.mf"))
    if "propro" in modules:
        cmdpro = "python /lustre/work/yongdeng/software/protokaryon/flow/propro.py -deg %s -vs %s -o %s -db %s -mf %s"%(join(analysis,"deg")," ".join([v.split(",")[0] for v in args.vs]),join(analysis,"propro"),args.prodb,join(out,"flow",now+"flow","Propro.mf"))
    ...
    if "asp" in modules:
        ...
        fa = abspath(args.fa)
        samplegtf = [s + "=" + g for s,g in zip(samples,[join(analysis,"stringtie",i,"transcripts.gtf.filter.gtf") for i in samples])] + ["merge=" + join(analysis,"stringtie","merge/merged.gtf")]
        cmdasp = "python /lustre/work/yongdeng/software/protokaryon/flow/asp.py -gtf %s -hdrs %s -o %s -mf %s"%(" ".join(samplegtf),fa+".hdrs",join(analysis,"asp"),join(out,"flow",now+"flow","Asp.mf"))
        os.system(cmdasp);shell.append(cmdasp);mf.append(join(out,"flow",now+"flow","Asp.mf"))
        
    if "enrichment" in modules:
        fa = abspath(args.fa)
        gtf = abspath(args.gtf)
        if args.nokobas:
            if args.abbr:
                print "Warning:for nokobas species, abbr seting is invalid"
            if args.database_type.rstrip("/") != "/K/G":
                print "ERROR:if nokabas specise, only /K/G database_type allowed"
                sys.exit(1)
            if not args.fa or not args.gtf or not args.species_type:
                print "ERROR:if nokabas specise, fa and gtf and species_type must be given"
                sys.exit(1)
            if args.noassemble:
                cmdgokegg = 'python /lustre/work/yongdeng/software/protokaryon/flow/enrichment2.py -deg %s -vs %s --nokobas -fa %s -gtf %s -st %s -o %s -mf %s'%(join(analysis,"deg")," ".join([v.split(",")[0] for v in args.vs]),fa,join(analysis,"stringtie","merge","merged.gtf"),args.species_type,join(analysis,"enrichment"),join(out,"flow",now+"flow","Enrichment.mf"))
            else:
                cmdgokegg = 'python /lustre/work/yongdeng/software/protokaryon/flow/enrichment2.py -deg %s -vs %s --nokobas -fa %s -gtf %s -st %s -o %s -mf %s'%(join(analysis,"deg")," ".join([v.split(",")[0] for v in args.vs]),fa,gtf,args.species_type,join(analysis,"enrichment"),join(out,"flow",now+"flow","Enrichment.mf"))
            os.system(cmdgokegg);shell.append(cmdgokegg);mf.append(join(out,"flow",now+"flow","Enrichment.mf"))
        else:
            cmdgokegg = "python /lustre/work/yongdeng/software/protokaryon/flow/enrichment.py -dt %s -it %s -deg %s -vs %s -abbr %s -o %s -mf %s"%(args.database_type,args.idtype,join(analysis,"deg")," ".join([v.split(",")[0] for v in args.vs]),args.abbr,join(analysis,"enrichment"),join(out,"flow",now+"flow","Enrichment.mf"))
            os.system(cmdgokegg);shell.append(cmdgokegg);mf.append(join(out,"flow",now+"flow","Enrichment.mf"))
...
#        sys.exit()
    ...
    if not (("raw" in modules) and ("clean" in modules) and ("align" in modules)):
        ...
        sys.exit(1)
    if "raw" in modules and "clean" in modules  and "align" in modules and "sum" in modules:
    ...
    if "diff" in modules:
        c+=1
        mkdir(join(result,str(c)+".differentially_expressed_gene"),[v.split(",")[0] for v in args.vs]+["venn_DEG","venn_DOWN","venn_UP"])
    ...
    if "cluster" in modules:  ## 缺少所有样本的一个聚类表,各分组对应的文件参考150631项目
        c +=1
        mkdir(join(result,str(c)+".cluster"),[v.split(",")[0] for v in args.vs])
        for i in [v.split(",")[0] for v in args.vs]:
...
#        cpdict[k] = v
    ...
    with open(join(args.outputdir,"results.sh"),"w") as fi:        
        ...
        if "enrichment" in modules:
            mkdir(join(result,"total.table.for.DEG"))
            fi.write(("python /lustre/work/yongdeng/software/protokaryon/flow/total.anno.py -sg %s -vs %s -anno %s -back %s -degdir %s -fpkm %s -o %s && "%(" ".join(args.sample_group)," ".join([v.split(",")[0] for v in args.vs]),join(analysis,"stringtie","track_ref.geneanno.txt"),join(analysis,"enrichment","back.anno.xls"),join(analysis,"deg"),join(analysis,"stringtie","gene_FPKM_matrix.txt"),join(analysis,"total.table.for.DEG.txt"))))
