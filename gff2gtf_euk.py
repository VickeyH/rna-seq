...
for line in os.popen('/lustre/work/yongdeng/software/protokaryon/soft/Align/gffread/gffread -T -F %s -o-'%sys.argv[1]):
    ...
    if len(trans) == 1:   # 一个基因只有一个转录本情况
        ...
        for line in gene_dict[i]:
            ...
            if pre[2] == "exon":
                ...
                product = change_product(product)
                p += 1
                protein_id = re.search('protein_id ".+?";',line).group() + " " if re.search('protein_id ".+?";',line) else 'protein_id "' + gene_id.split('"')[1] + '_P%d";'%p + " "
